...
class _CONSTRUCT(ASN1Obj):
    ...
    def _safechk_bnd(self, val, parent_key=''):
        ...
        for (name, Obj) in self._cont.items():
            if name in val:
                Obj._val = val[name]
                child_key = '{0}.{1}'.format(_key, name) if _key else name
                Obj._safechk_bnd(Obj._val, child_key)
                ...
                if Bv & (1<<(ldet-1-i)):
                    ...
                    if i < len(self._ext_nest):
                        ...
                        if isinstance(ext, list):
                            ...
                            grp_val, _gen = ASN1CodecPER.decode_unconst_open_ws(char, wrapped=Comp)
                            val.update(grp_val)
                        else:
                            ...
                            _par = Comp._parent
                            Comp._parent = self
                            ext_val, _gen = ASN1CodecPER.decode_unconst_open_ws(char, wrapped=Comp)
                            Comp._parent = _par
                            val[ext] = ext_val
                    else:
                        # unknown extension
                ...
                if Bv & (1<<(ldet-1-i)):
                    ...
                    if i < len(self._ext_nest):
                        ...
                        if isinstance(ext, list):
                            # grouped extension
                            Comp = self._ext_group_obj[self._ext_ident[ext[0]]]
                            val.update(ASN1CodecPER.decode_unconst_open(char, wrapped=Comp))
                        else:
                            # single extension, ident == ext
                            Comp = self._cont[ext]
                            _par = Comp._parent
                            Comp._parent = self
                            val[ext] = ASN1CodecPER.decode_unconst_open(char, wrapped=Comp)
                            Comp._parent = _par
                    else:
                        # unknown extension
                        val['_ext_%r' % i] = ASN1CodecPER.decode_unconst_open(char)
        #
        self._val = val
        return
                    ...
                    if i < len(self._ext_nest):
                        ...
                        if isinstance(ext, list):
                            ...
                            if Comp:
                                ...
                                val.update(Comp._val)
                            else:
                                val.update(val_bytes)
                        else:
                            # single extension, ident == ext
                            Comp = self._cont[ext]
                            _par = Comp._parent
    ...
    def _from_oer_ws(self, char):
        ...
        if extended:
            ...
            ldet = ldet - pad_bits
            #
            GEN.append(Envelope('Extension-bmp', GEN=(ldet_struct, i_oct, ext_bmp)))
                ...
                if Bv & (1<<(ldet-1-i)):
                    ...
                    if i < len(self._ext_nest):
                        ...
                        if isinstance(ext, list):
                            # grouped extension
                            Comp = self._ext_group_obj[self._ext_ident[ext[0]]]
                            val_bytes, _struct = ASN1CodecOER.decode_open_type_ws(char)
                            if Comp:
                                Comp.from_oer(val_bytes)
                                val.update(Comp._val)
                            else:
...
class _CONSTRUCT_OF(ASN1Obj):
    ...
    def _safechk_val(self, val, parent_key=''):
        ...
        for i, v in enumerate(val):
            ...
            self._cont._safechk_val(v, item_key)
